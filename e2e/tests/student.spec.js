...
test.describe("Student (Arthur) browser tests", () => {
  test.beforeAll(() => {
    test.skip(!getSessionId(ROLE), "No STUDENT session available");
  });
  ...
  test("identity / role verified via /api/auth/me/", async ({ browser }) => {
    ...
    expect(me.username).toBe("SA-ST-0001");
    const roles = (me.memberships || []).flatMap((m) => (m.roles || []).map((r) => r));
    expect(roles).toContain("student");
    await context.close();
  });

  test("dashboard renders for student", async ({ browser }) => {
    const { context, page } = await authenticatedPage(browser, ROLE);
    await page.goto("/", { waitUntil: "domcontentloaded" });
    await waitForTopbar(page);
  ...
  test("backend blocks payroll API (403)", async ({ browser }) => {
    const { context, page } = await authenticatedPage(browser, ROLE);
    const resp = await page.request.get("/api/payroll/records/");
    expect(resp.status(), "expected 403 for GET /api/payroll/records/").toBe(403);
    await context.close();
  });

  test("backend scopes finance, staff & exams reads (no 500)", async ({ browser }) => {
    const { context, page } = await authenticatedPage(browser, ROLE);
    for (const path of ["/api/finance/invoices/", "/api/finance/payments/", "/api/staff/", "/api/exams/"]) {
