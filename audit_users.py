...
backend_dir = r'C:\Users\Ryuk\Documents\perfect-foundation-sms\backend'
...
# Get all institution memberships with roles
...
# Print user reports
print(f"{'Username':<20} {'Email':<30} {'Active':<8} {'Superuser':<10} {'Primary Role':<15} {'Memberships':<25} {'Role Assignments':<30}")
print("-" * 160)
...
for username, data in sorted(user_data.items()):
    ...
    print(f"{username:<20} {str(data['email'])[:30]:<30} {str(data['is_active']):<8} {str(data['is_superuser']):<10} {str(data['primary_role'])[:15]:<15} {memberships_str:<25} {roles_str:<30}")
