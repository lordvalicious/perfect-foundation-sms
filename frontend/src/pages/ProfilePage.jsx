...
function ProfileHero({ photoUrl, name, username, badges }) {
  return (
...
export default function ProfilePage() {
        ...
        if (type === "teacher") {
          ...
          if (user.teacher_profile_id) {
            ...
            if (tRes.ok) {
              ...
              if (!cancelled) {
                setProfile({ ...t, kind: "teacher" });
              }
            }
          } else if (user.student_profile_id) {
            const sRes = await fetch("/api/students/me/", {
              credentials: "include",
            });
  ...
  const profileName =
    profile?.full_name || profile?.first_name || account?.first_name || "";
  const fullName = `${account?.first_name || ""} ${
    account?.last_name || ""
  }`.trim();

  const name = profile ? profileName : fullName || account?.username;
      ...
      {!loading && !error && (
        <>
          <ProfileHero
            ...
            name={name}
            username={profile ? undefined : account?.username}
            badges={roleBadges}
          />
